...
class M_Judul extends CI_Model{
    ...
    {
        if($prodi==0){
          $query=$this->db->query("SELECT *, mahasiswa.nama as nama_mahasiswa, mahasiswa.npm as npm_mahasiswa FROM mahasiswa LEFT JOIN ta on ta.npm=mahasiswa.npm  left join prodi on prodi.id_prodi=mahasiswa.id_prodi left join bidang_minat on bidang_minat.id_bidang_minat=mahasiswa.id_bidang_minat where ta.verifikasi='1'and  (ta.sampai is null or ta.sampai<= now()) and (lower(mahasiswa.nama) LIKE lower('%$id%')  OR lower(ta.judul) like lower('%$id%') or lower(bidang_minat.nama_bidang_minat) like lower('%$id%') or lower(ta.Abstrak) like lower('%$id%'))");
        }else{
